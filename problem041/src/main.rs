fn main() {
    ...
    while n>=9 {
        if n<max { n=2; }
        if is_pandigital(n) {
            println!("Found pandigital {}", n);
            if is_prime(&mut primes,n) {
                println!("Is also prime");
                if n > max {
                    max = n;
                    println!("New max: {}", max);
                }
...
fn generate_primes(primes: &mut Vec<u128>, limit: u128) {
    ...
    while n <= limit {
        if is_prime(primes, n) { primes.push(n); }
        n += 2;
    }
}
...
fn is_pandigital(n: u128) -> bool {
    ...
    while temp_n > 0 {
        let digit = temp_n % 10;
        if digit> digits.len() as u128 { return false; }
        if digit == 0 || digits[digit as usize - 1] { return false; }
