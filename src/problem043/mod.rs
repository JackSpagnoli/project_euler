...
fn iterate_2()->u128{
    ...
    while d2!=9 || d3!=8 || d4!=6{
        ...
        if !used_digits[d3 as usize] {
            used_digits[d3 as usize]=true;
            if !used_digits[d4 as usize] {
...
fn iterate(iterators:[u8;6], index:usize, used_digits:[bool;10], digits:[u8;10])->u128{
    ...
    while digits[index+4]<10 {
        if !used_digits[digits[index+4] as usize] && (prior+digits[index+4] as u16) % iterators[index] as u16 == 0 {
            let mut used_digits = used_digits;
