...
fn permutation(a: &u64, b: &u64) -> bool {
    ...
    if !digits_a.contains(&(b % 10)) { return false; }
    if !digits_a.contains(&((b / 10) % 10)) { return false; }
    if !digits_a.contains(&((b / 100) % 10)) { return false; }
    if !digits_a.contains(&((b / 1000) % 10)) { return false; }

    let digits_b: [u64; 4] = [b % 10, (b / 10) % 10, (b / 100) % 10, (b / 1000) % 10];

    if !digits_b.contains(&(a % 10)) { return false; }
    if !digits_b.contains(&((a / 10) % 10)) { return false; }
    if !digits_b.contains(&((a / 100) % 10)) { return false; }
    if !digits_b.contains(&((a / 1000) % 10)) { return false; }
